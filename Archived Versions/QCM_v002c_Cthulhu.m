function varargout = QCM_v002c_Cthulhu(varargin)
%This QCM Program was created by Chyi-Huey Joshua Yeh from the Shull
%Research Group at Northwestern University.
% QCM_V002C_CTHULHU MATLAB code for QCM_v002c_Cthulhu.fig
%      QCM_V002C_CTHULHU, by itself, creates a new QCM_V002C_CTHULHU or raises the existing
%      singleton*.
%
%      H = QCM_V002C_CTHULHU returns the handle to a new QCM_V002C_CTHULHU or the handle to
%      the existing singleton*.
%
%      QCM_V002C_CTHULHU('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in QCM_V002C_CTHULHU.M with the given input arguments.
% 
%      QCM_V002C_CTHULHU('Property','Value',...) creates a new QCM_V002C_CTHULHU or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before QCM_v002c_Cthulhu_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to QCM_v002c_Cthulhu_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help QCM_v002c_Cthulhu

% Last Modified by GUIDE v2.5 11-Aug-2015 13:38:54

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @QCM_v002c_Cthulhu_OpeningFcn, ...
                   'gui_OutputFcn',  @QCM_v002c_Cthulhu_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before QCM_v002c_Cthulhu is made visible.
function QCM_v002c_Cthulhu_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to QCM_v002c_Cthulhu (see VARARGIN)

% Choose default command line output for QCM_v002c_Cthulhu
...
%Create a lend box for the primaryaxes1 and primaryaxes2
...
% UIWAIT makes QCM_v002c_Cthulhu wait for user response (see UIRESUME)
% uiwait(handles.primary1);


% --- Outputs from this function are returned to the command line.
function varargout = QCM_v002c_Cthulhu_OutputFcn(~, ~, handles) 
   ...
   if isempty(harm_tot)%if there are no harmonics selected to be recorded by the user, default to recording the first harmonic
    ...
    if handles.din.refit_flag==0%run this if collecting dynamic data
        ...
        try
            ...
            temp=handles.din.refit.(rawfile){handles.din.refit.counter};%extract out the timepoints
            timestamp=temp(handles.din.refit_time_index:end-10);
            time_elapsed=strrep(timestamp,'dot','.');%time in minutes
            my_disp('---------------------\n',[0 0.5 0]);
            my_disp('Timestamp: ','text');
            my_disp([num2str(time_elapsed),' min\n'],'blue'); 
            set(handles.last_time,'string',['Last timepoint: ',num2str(time_elapsed),' min']); 
                    ...
                    if get(handles.dynamic_fit,'value')==1
                        ...
                        if get(handles.radio_chi,'value')==1 %determine whether or not to show chi squared parameters next to plots
                            ...
                            set(handles.(xsq_name),'visible','on','string',...
                                ['Xsq = ',num2str(handles.din.chi_sqr_value(n,harm_tot(dum)+1))],'fontsize',8);
...
if handles.din.refit_flag==0%run this if collecting live data
    ...
    if handles.din.refit.counter<=length(handles.din.refit_timepoints)&&...
        ...
        handles.mdin.refit.counter<=str2double(get(handles.refit_end,'string'))&&...
...
for dum=1:2:11
    ...
    handles.prefs.peak_min(dum)=data(0.5*(dum+1),1);
    handles.prefs.num_peaks(dum)=data(0.5*(dum+1),1);
end%for dum=1:2:11
guidata(hObject,handles);
