...
void CAccessMyVNADlg::OnBnClickedButtonSingle()
{
	...
	do{
		...
		CAccessMyVNADlg::OnBnClickedButtonSavescan();
		n++;
	} while (n < 1);
...
{
	//CFileDialog  dlgFile(FALSE, _T("txt"), NULL, OFN_EXPLORER | OFN_ENABLESIZING, _T("S parameter data (*.txt)|*.txt|All Files (*.*)|*.*||"));
	//CString fileName("C:\\Users\\Josh\\Desktop\\myVNAconfig.txt");
	//dlgFile.GetOFN().lpstrFile = fileName.GetBuffer(4096);
	//dlgFile.GetOFN().nMaxFile = 4096;
	//INT_PTR nResult = dlgFile.DoModal();
	//if (nResult == IDOK)
	...
	double dData[200];
	double dData2[200];
	MyVNAGetScanData(0, 199, -2, 16, &dFreq[0], &dData2[0]);
	MyVNAGetScanData(0, 199, DISPLAY_FREQ_SCALE, 15, &dFreq[0], &dData[0]);
	ofstream f1("C:\\Users\\Josh\\Desktop\\vnaoutput\\myVNAfreq.csv"); // the output file for frequency
	copy(dFreq, dFreq + 200, ostream_iterator<double>(f1, "\n"));
	ofstream f2("C:\\Users\\Josh\\Desktop\\vnaoutput\\myVNAdata.csv"); // the output file for first dataset
	copy(dData, dData + 200, ostream_iterator<double>(f2, "\n"));
	ofstream f3("C:\\Users\\Josh\\Desktop\\vnaoutput\\myVNAdata2.csv"); // the output file for first dataset
	copy(dData2, dData2 + 200, ostream_iterator<double>(f3, "\n"));
